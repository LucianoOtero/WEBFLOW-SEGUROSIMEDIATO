 ...
 * - URLs atualizadas para endpoints _prod.php
 * 
...
$(function() {
   ...
   * @param {string} endpoint - 'flyingdonkeys' ou 'octadesk'
  ...
  function getEndpointUrl(endpoint) {
    ...
    const endpoints = {
      ...
      flyingdonkeys: '/add_flyingdonkeys.php'
    };
    
    const endpointPath = endpoints[endpoint] || '/add_flyingdonkeys.php';
    return window.APP_BASE_URL + endpointPath;
  }
  ...
  function saveLeadState(leadData) {
    ...
    try {
      ...
      try {
        ...
        if (typeof window._whatsappModalLeadState === 'undefined') {
  ...
  function getLeadState() {
    // Tentar ler de localStorage, com fallback para sessionStorage ou memória (FASE 11 - Correção MÉDIA)
    try {
      ...
      // Fallback 1: Tentar sessionStorage
      try {
        const stored = sessionStorage.getItem('whatsapp_modal_lead_state');
        if (stored) {
          const state = JSON.parse(stored);
          // Verificar expiração
          if (Date.now() > state.expires) {
        ...
        // Fallback 2: Ler de memória (variável global)
        if (typeof window._whatsappModalLeadState !== 'undefined' && window._whatsappModalLeadState) {
          const state = window._whatsappModalLeadState;
          // Verificar expiração
          if (Date.now() > state.expires) {
            window._whatsappModalLeadState = {};
            return null;
  ...
  $(MODAL_CONFIG.fieldIds.cep).on('blur', function() {
    ...
    $.getJSON(`${VIACEP_BASE_URL}/ws/${cepDigits}/json/`) // FASE 3 - Correção ALTA: usar constante configurável
      .done(function(data) {
