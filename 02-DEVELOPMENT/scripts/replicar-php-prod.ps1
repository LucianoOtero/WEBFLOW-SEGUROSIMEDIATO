# Script para replicar arquivo PHP de DEV para PROD com validação completa
# Versão: 1.0.0
# Data: 21/11/2025
# Última Correção: -
# Uso: .\replicar-php-prod.ps1 arquivo.php
#
...
Write-Host "REPLICACAO SEGURA: PHP DEV -> PROD" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
# FASE 7: Validar sintaxe PHP em PROD
Write-Host "🔍 FASE 7: Validando sintaxe PHP em PROD..." -ForegroundColor Cyan
$sintaxeProd = ssh $servidorProd "cd /var/www/html/prod/root && php -l $arquivo"
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ ERRO: Sintaxe PHP inválida em PROD:" -ForegroundColor Red
    Write-Host $sintaxeProd -ForegroundColor Red
    Write-Host ""
    Write-Host "⚠️ Restaurando backup..." -ForegroundColor Yellow
    ssh $servidorProd "mv ${caminhoProd}.backup_${timestamp} $caminhoProd" | Out-Null
    exit 1
}
Write-Host "✅ Sintaxe PHP válida em PROD" -ForegroundColor Green
Write-Host ""
...
Write-Host "1. Testar funcionalidade em PROD" -ForegroundColor White
Write-Host "2. Verificar logs do servidor" -ForegroundColor White
Write-Host "3. Atualizar documento de tracking" -ForegroundColor White
