...
(function() {
    ...
    // ======================
    // CONSTANTES DE ENDPOINTS E URLs (FASE 3b - Correção ALTA)
    // ======================
    ...
    // APIs Externas (usando variáveis de ambiente - sem fallbacks críticos)
    ...
    if (!window.RPA_API_BASE_URL) {
      throw new Error('[CONFIG] ERRO CRÍTICO: RPA_API_BASE_URL não está definido. Carregue config_env.js.php ANTES deste script.');
    }
    if (!window.SUCCESS_PAGE_URL) {
      throw new Error('[CONFIG] ERRO CRÍTICO: SUCCESS_PAGE_URL não está definido. Deve ser definido via data attributes no FooterCodeSiteDefinitivoCompleto.js');
    }
    
    const VIACEP_BASE_URL = window.VIACEP_BASE_URL;
    const APILAYER_BASE_URL = window.APILAYER_BASE_URL;
    const SAFETYMAILS_OPTIN_BASE = window.SAFETYMAILS_OPTIN_BASE;
    // SAFETYMAILS_OPTIN_PATH pode ter fallback (não crítico)
    ...
    // APIs RPA e Redirecionamento
    const RPA_API_BASE_URL = window.RPA_API_BASE_URL;
    const SUCCESS_PAGE_URL = window.SUCCESS_PAGE_URL;
    ...
    class SpinnerTimer {
        ...
        init() {
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'UI_TRACE', 'Inicializando SpinnerTimer', null, 'UI', 'SIMPLE');
                window.novo_log('TRACE', 'UI_TRACE', 'Elementos do spinner', { 
                    spinnerCenter: !!this.elements.spinnerCenter, 
                    timerMessage: !!this.elements.timerMessage 
                }, 'UI', 'MEDIUM');
            }
            
            if (!this.elements.spinnerCenter) {
                if (window.novo_log) {
                    window.novo_log('WARN', 'UI', 'Elementos do spinner timer não encontrados', null, 'UI', 'SIMPLE');
                }
                return;
            }
            
            if (window.novo_log) {
                window.novo_log('DEBUG', 'UI_TRACE', 'Iniciando timer', null, 'UI', 'SIMPLE');
            }
            this.start();
        }
        ...
        start() {
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'UI_TRACE', 'Timer iniciado', { seconds: this.remainingSeconds }, 'UI', 'MEDIUM');
            }
        ...
        updateDisplay() {
            ...
            if (this.elements.spinnerCenter) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Timer atualizado', { timerText }, 'UI', 'SIMPLE');
                }
            } else {
                if (window.novo_log) {
                    window.novo_log('WARN', 'UI', 'spinnerCenter não encontrado para atualizar', null, 'UI', 'SIMPLE');
                }
            }
        }
    ...
    class ProgressModalRPA {
        constructor(sessionId) {
            this.apiBaseUrl = RPA_API_BASE_URL; // FASE 2 - Correção ALTA: usar constante configurável
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'ProgressModalRPA inicializado', { sessionId: this.sessionId }, 'INIT', 'MEDIUM');
            }
        }
        ...
        setSessionId(sessionId) {
            this.sessionId = sessionId;
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'SessionId atualizado', { sessionId: this.sessionId }, 'OPERATION', 'MEDIUM');
            }
        ...
        initSpinnerTimer() {
            ...
            if (!this.spinnerTimer) {
                ...
                // Log para debug
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'UI_TRACE', 'SpinnerTimer inicializado e iniciado', null, 'UI', 'SIMPLE');
                }
            }
        }
        ...
        stopSpinnerTimer() {
            ...
            try {
                ...
                if (this.spinnerTimer) {
                    ...
                    // Log para debug
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'UI_TRACE', 'SpinnerTimer parado', null, 'UI', 'SIMPLE');
                    }
                }
                ...
                if (spinnerContainer) {
                    ...
                    // Log para debug
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'UI_TRACE', 'Spinner timer escondido', null, 'UI', 'SIMPLE');
                    }
                }
            } catch (error) {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'RPA', 'Erro ao parar spinner timer', error, 'ERROR_HANDLING', 'MEDIUM');
                }
                ...
                if (container) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'RPA', 'Spinner escondido via fallback', null, 'ERROR_HANDLING', 'SIMPLE');
                    }
                }
            }
        }
        ...
        startProgressPolling() {
            if (!this.sessionId) {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'RPA', 'Session ID não encontrado', null, 'ERROR_HANDLING', 'SIMPLE');
                }
                return;
            }
            
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Iniciando polling do progresso', null, 'OPERATION', 'SIMPLE');
            }
            this.pollCount = 0;
            ...
            this.progressInterval = setInterval(() => {
                this.pollCount++;
                if (window.novo_log) {
                    window.novo_log('TRACE', 'POLLING_TRACE', `Polling ${this.pollCount}/${this.maxPolls}`, null, 'PERFORMANCE', 'SIMPLE');
                }
                
                if (this.pollCount > this.maxPolls) {
                    if (window.novo_log) {
                        window.novo_log('ERROR', 'RPA', 'Timeout: Processamento demorou mais de 10 minutos', null, 'ERROR_HANDLING', 'SIMPLE');
                    }
        ...
        stopProgressPolling() {
            if (this.progressInterval) {
                ...
                this.progressInterval = null;
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'POLLING_TRACE', 'Polling interrompido', null, 'PERFORMANCE', 'SIMPLE');
                }
            }
        }
        ...
        async updateProgress() {
            ...
            try {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'PROGRESS_TRACE', 'Dados do progresso recebidos', {
                        etapa_atual: data.progress?.etapa_atual,
                        fase_atual: data.progress?.fase_atual,
                        status: data.progress?.status,
                        mensagem: data.progress?.mensagem,
                        error_code: data.progress?.error_code,
                        status_code: data.progress?.status_code
                    }, 'DATA_FLOW', 'VERBOSE');
                }
                ...
                if (data.success) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'PROGRESS_TRACE', 'Dados completos do progresso', {
                    ...
                    if (this.isErrorStatus(currentStatus, mensagem, errorCode)) {
                        if (window.novo_log) {
                            window.novo_log('ERROR', 'RPA', 'Erro detectado no RPA', { 
                    ...
                    if (currentStatus === 'success') {
                        currentPhase = 16;
                        if (window.novo_log) {
                            window.novo_log('INFO', 'RPA', 'Status success detectado → forçando fase 16 (finalização completa)', null, 'OPERATION', 'SIMPLE');
                        }
                    }
                    ...
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'PROGRESS_TRACE', `Fase ${currentPhase}: ${percentual}%`, { 
                            fase: currentPhase, 
                            percentual, 
                            status: currentStatus 
                        }, 'DATA_FLOW', 'MEDIUM');
                    ...
                    if (progressData.dados_extra || currentStatus === 'success') {
                        ...
                        if (currentStatus === 'success') {
                            if (window.novo_log) {
                                window.novo_log('INFO', 'RPA', 'RPA concluído com sucesso!', null, 'OPERATION', 'SIMPLE');
                            }
                            this.stopProgressPolling();
        ...
        handleRPAError(mensagem, errorCode = null) {
            if (window.novo_log) {
                window.novo_log('ERROR', 'RPA', 'Tratando erro do RPA', { mensagem, errorCode }, 'ERROR_HANDLING', 'MEDIUM');
            }
        ...
        updateProgressElements(percentual, currentPhase, currentStatus, progressData, totalEtapas = 16) {
            if (window.novo_log) {
                window.novo_log('TRACE', 'UI_TRACE', 'Atualizando elementos do progresso', { 
                    percentual, 
                    fase: currentPhase, 
                    status: currentStatus 
                }, 'UI', 'MEDIUM');
            ...
            if (progressText) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Progress text atualizado', { text: progressText.textContent }, 'UI', 'SIMPLE');
                }
            }
            ...
            if (currentPhaseElement) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Current phase atualizado', { message }, 'UI', 'SIMPLE');
                }
            }
            ...
            if (subPhaseElement) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Sub phase atualizado', { subMessage }, 'UI', 'SIMPLE');
                }
            }
            ...
            if (stageInfo) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Stage info atualizado', { text: stageInfo.textContent }, 'UI', 'SIMPLE');
                }
            }
            ...
            if (progressFill) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Progress fill atualizado', { width: progressFill.style.width }, 'UI', 'SIMPLE');
                }
            }
        }
        ...
        updateInitialEstimate(data) {
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'DATA_TRACE', 'Atualizando estimativa inicial', null, 'DATA_FLOW', 'SIMPLE');
            }
            ...
            if (window.novo_log && estimativas) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Estimativas encontradas', { hasEstimativas: !!estimativas }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            if (estimativas) {
                ...
                if (valorInicial) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'DATA_TRACE', 'Valor inicial formatado', { valorFormatado }, 'DATA_FLOW', 'MEDIUM');
                    }
                    ...
                    if (window.novo_log) {
                        window.novo_log('WARN', 'DATA_TRACE', 'Valor inicial não encontrado nas estimativas', null, 'DATA_FLOW', 'SIMPLE');
                    }
                }
            } else {
                if (window.novo_log) {
                    window.novo_log('WARN', 'DATA_TRACE', 'Nenhuma estimativa encontrada nos dados', null, 'DATA_FLOW', 'SIMPLE');
                }
            }
        }
        ...
        updateResults(data) {
            if (window.novo_log) {
                window.novo_log('DEBUG', 'DATA_TRACE', 'Atualizando resultados finais', null, 'DATA_FLOW', 'SIMPLE');
                window.novo_log('TRACE', 'DATA_TRACE', 'Estrutura completa dos dados', data, 'DATA_FLOW', 'VERBOSE');
            }
            ...
            if (data.resultados_finais?.dados?.dados_finais) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Dados encontrados em resultados_finais.dados.dados_finais', null, 'DATA_FLOW', 'SIMPLE');
                }
            }
            ...
            if (!planoRecomendado && data.timeline) {
                ...
                if (finalEntry?.dados_extra) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', 'Dados encontrados em timeline[final].dados_extra', null, 'DATA_FLOW', 'SIMPLE');
                    }
                }
            }
            ...
            if (!planoRecomendado && data.dados_extra) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Dados encontrados em dados_extra direto', null, 'DATA_FLOW', 'SIMPLE');
                }
            }
            
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Estrutura completa dos dados', {
            ...
            if (planoRecomendado && planoAlternativo) {
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'DATA_TRACE', 'Resultados encontrados', { 
                        hasRecomendado: !!planoRecomendado, 
                        hasAlternativo: !!planoAlternativo 
                    }, 'DATA_FLOW', 'MEDIUM');
                }
                ...
                if (window.novo_log) {
                    window.novo_log('WARN', 'DATA_TRACE', 'Nenhum resultado final encontrado em nenhuma estrutura', null, 'DATA_FLOW', 'SIMPLE');
                }
            }
        }
        ...
        updateCardValue(elementId, valor) {
            if (window.novo_log) {
                window.novo_log('TRACE', 'UI_TRACE', `updateCardValue chamado`, { elementId, valor, tipo: typeof valor }, 'UI', 'MEDIUM');
            }
            
            if (valor) {
                const element = document.querySelector(`#rpaModal #${elementId}`);
                if (window.novo_log && element) {
                    window.novo_log('TRACE', 'UI_TRACE', 'Elemento encontrado', { elementId }, 'UI', 'SIMPLE');
                }
                ...
                if (element) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'UI_TRACE', `Valor ${elementId} atualizado`, { valorFormatado }, 'UI', 'MEDIUM');
                    }
                } else {
                    if (window.novo_log) {
                        window.novo_log('ERROR', 'UI', `Elemento #${elementId} não encontrado no DOM`, null, 'UI', 'SIMPLE');
                    }
                }
            } else {
                if (window.novo_log) {
                    window.novo_log('WARN', 'UI', `Valor vazio para ${elementId}`, { valor }, 'UI', 'SIMPLE');
                }
            }
        }
        ...
        updateCardDetails(prefix, plano) {
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'UI_TRACE', `Atualizando detalhes do plano ${prefix}`, { prefix }, 'UI', 'SIMPLE');
            }
        ...
        updateField(elementId, value) {
            ...
            if (element) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'UI_TRACE', `Campo ${elementId} atualizado`, { value }, 'UI', 'SIMPLE');
                }
            }
        }
        ...
        formatCurrency(value) {
            ...
            if (typeof value === 'string' && value.includes('R$')) {
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Valor já formatado', { value }, 'DATA_FLOW', 'SIMPLE');
                }
                return value;
            }
            ...
            if (isNaN(numericValue)) {
                if (window.novo_log) {
                    window.novo_log('WARN', 'DATA_TRACE', 'Valor inválido para formatação', { value }, 'DATA_FLOW', 'SIMPLE');
                }
                return 'R$ 0,00';
            }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Valor formatado', { value, formatted }, 'DATA_FLOW', 'MEDIUM');
            }
            return formatted;
        }
    }
    ...
    class FormValidator {
        constructor() {
            this.config = {
                USE_PHONE_API: window.USE_PHONE_API || (function() {
                  throw new Error('[CONFIG] ERRO CRÍTICO: USE_PHONE_API não está definido. Deve ser definido via data attributes no FooterCodeSiteDefinitivoCompleto.js');
                })(),
                APILAYER_KEY: window.APILAYER_KEY || (function() {
                ...
                VALIDAR_PH3A: window.VALIDAR_PH3A !== undefined ? window.VALIDAR_PH3A : (function() {
                  throw new Error('[CONFIG] ERRO CRÍTICO: VALIDAR_PH3A não está definido. Deve ser definido via data attributes no FooterCodeSiteDefinitivoCompleto.js');
                })()
            };
        }
        ...
        async validatePlaca(placa) {
            ...
            try {
                if (!window.APP_BASE_URL) {
                    if (window.novo_log) {
                        window.novo_log('ERROR', 'CONFIG', 'APP_BASE_URL não disponível para validação de placa', null, 'INIT', 'SIMPLE');
                    }
        ...
        validarCelularLocal(ddd, numero) {
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'validarCelularLocal iniciado', { ddd, numero }, 'DATA_FLOW', 'MEDIUM');
            }
            const d = this.onlyDigits(ddd), n = this.onlyDigits(numero);
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'DDD e número limpos', { d, n, dLength: d.length, nLength: n.length }, 'DATA_FLOW', 'MEDIUM');
            }
            
            if (d.length !== 2) {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'VALIDACAO', 'DDD inválido', { length: d.length }, 'DATA_FLOW', 'SIMPLE');
                }
                return { ok: false, reason: 'ddd' };
            }
            if (n.length !== 9) {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'VALIDACAO', 'Numero inválido - length incorreto', { length: n.length, esperado: 9 }, 'DATA_FLOW', 'SIMPLE');
                }
                return { ok: false, reason: 'len' };
            }
            if (n[0] !== '9') {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'VALIDACAO', 'Numero não começa com 9', { primeiroDigito: n[0] }, 'DATA_FLOW', 'SIMPLE');
                }
                return { ok: false, reason: 'pattern' };
            }
            if (window.novo_log) {
                window.novo_log('DEBUG', 'VALIDACAO', 'Celular válido', { national: d + n }, 'DATA_FLOW', 'MEDIUM');
            }
        ...
        async validateCelular(ddd, celular) {
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'validateCelular iniciado', { ddd, celular }, 'DATA_FLOW', 'MEDIUM');
            }
            const local = this.validarCelularLocal(ddd, celular);
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Resultado validação local', local, 'DATA_FLOW', 'MEDIUM');
            }
            
            if (!local.ok) {
                if (window.novo_log) {
                    window.novo_log('ERROR', 'VALIDACAO', 'Validação local falhou', { reason: local.reason }, 'DATA_FLOW', 'SIMPLE');
                }
                return { ok: false, reason: local.reason };
            }
            
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Validando via API', null, 'DATA_FLOW', 'SIMPLE');
            }
            const api = await this.validarCelularApi(local.national);
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Resultado validação API', api, 'DATA_FLOW', 'MEDIUM');
            }
            
            const result = { ok: api.ok };
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Resultado final validateCelular', result, 'DATA_FLOW', 'MEDIUM');
            }
            return result;
        }
    ...
    class MainPage {
        ...
        init() {
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'MainPage inicializada', null, 'INIT', 'SIMPLE');
            }
        ...
        setupFormSubmission() {
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Formulários encontrados', { count: forms.length }, 'INIT', 'MEDIUM');
            }
            ...
            if (submitButton) {
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Botão submit_button_auto encontrado', null, 'INIT', 'SIMPLE');
                }
                ...
                submitButton.addEventListener('click', (e) => {
                    ...
                    e.stopPropagation();
                    if (window.novo_log) {
                        window.novo_log('INFO', 'RPA', 'Botão CALCULE AGORA! clicado', null, 'OPERATION', 'SIMPLE');
                    }
                    ...
                    if (form) {
                        if (window.novo_log) {
                            window.novo_log('DEBUG', 'RPA', 'Formulário encontrado via botão', null, 'OPERATION', 'SIMPLE');
                        }
                        this.handleFormSubmit(form);
                    } else {
                        if (window.novo_log) {
                            window.novo_log('ERROR', 'RPA', 'Formulário não encontrado', null, 'ERROR_HANDLING', 'SIMPLE');
                        }
                    }
                });
            }
            ...
            forms.forEach((form, index) => {
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', `Configurando formulário ${index + 1}`, null, 'INIT', 'SIMPLE');
                }
                
                form.addEventListener('submit', (e) => {
                    e.preventDefault();
                    if (window.novo_log) {
                        window.novo_log('INFO', 'RPA', 'Formulário submetido', null, 'OPERATION', 'SIMPLE');
                    }
                    this.handleFormSubmit(form);
                });
            });
        ...
        collectFormData(form) {
            ...
            if (gclidField) {
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Campo GCLID_FLD capturado', { GCLID_FLD: data.GCLID_FLD }, 'DATA_FLOW', 'MEDIUM');
                }
            } else {
                data.GCLID_FLD = 'TesteRPA123'; // Valor padrão
                if (window.novo_log) {
                    window.novo_log('WARN', 'RPA', 'Campo GCLID_FLD não encontrado, usando valor padrão', null, 'DATA_FLOW', 'SIMPLE');
                }
            }
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Dados coletados', { 
                    campos: Object.keys(completeData).length 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            return completeData;
        }
        ...
        removeDuplicateFields(data) {
            ...
            fieldsToRemove.forEach(field => {
                if (cleanedData[field]) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', `Removendo campo duplicado incorreto: ${field}`, null, 'DATA_FLOW', 'SIMPLE');
                    }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Campos duplicados removidos', { 
                    camposRestantes: Object.keys(cleanedData).length 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            return cleanedData;
        }
        ...
        applyFieldConversions(data) {
            ...
            if (data['ESTADO-CIVIL']) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Estado civil convertido', { 
                        original: data['ESTADO-CIVIL'], 
                        convertido: data.estado_civil 
                    }, 'DATA_FLOW', 'MEDIUM');
                }
            }
            ...
            if (data.SEXO) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Sexo convertido', { 
                        original: data.SEXO, 
                        convertido: data.sexo 
                    }, 'DATA_FLOW', 'MEDIUM');
                }
            }
            ...
            if (data['TIPO-DE-VEICULO']) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Tipo de veículo convertido', { 
                        original: data['TIPO-DE-VEICULO'], 
                        convertido: data.tipo_veiculo 
                    }, 'DATA_FLOW', 'MEDIUM');
                }
            }
            ...
            if (data['DDD-CELULAR'] && data.CELULAR && !data.telefone) {
                ...
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Telefone concatenado', { 
                        ddd: data['DDD-CELULAR'], 
            ...
            if (data['TIPO-DE-VEICULO']) {
                data.tipo_veiculo = this.convertTipoVeiculo(data['TIPO-DE-VEICULO']);
                if (window.novo_log) {
                    window.novo_log('TRACE', 'DATA_TRACE', 'Tipo de veículo convertido (após mapeamento)', { 
                        original: data['TIPO-DE-VEICULO'], 
                        convertido: data.tipo_veiculo 
                    }, 'DATA_FLOW', 'MEDIUM');
                }
        ...
        convertEstadoCivil(webflowValue) {
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Convertendo estado civil', { 
                    original: webflowValue, 
                    normalized 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            for (const [key, value] of Object.entries(estadoCivilMapping)) {
                if (normalized.includes(key) || key.includes(normalized)) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', 'Correspondência parcial encontrada', { 
                            normalized, 
                            value 
                        }, 'DATA_FLOW', 'MEDIUM');
                    }
            ...
            if (window.novo_log) {
                window.novo_log('WARN', 'DATA_TRACE', 'Estado civil não mapeado, usando fallback', { 
                    webflowValue 
                }, 'DATA_FLOW', 'SIMPLE');
            }
        ...
        convertSexo(webflowValue) {
            const normalized = webflowValue.toLowerCase().trim();
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Convertendo sexo', { 
                    original: webflowValue, 
                    normalized 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            for (const [key, value] of Object.entries(sexoMapping)) {
                if (normalized.includes(key) || key.includes(normalized)) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', 'Correspondência parcial encontrada (sexo)', { 
                            normalized, 
                            value 
                        }, 'DATA_FLOW', 'MEDIUM');
                    }
            ...
            // Fallback: retornar valor original capitalizado
            if (window.novo_log) {
                window.novo_log('WARN', 'DATA_TRACE', 'Sexo não mapeado, usando fallback', { 
                    webflowValue 
                }, 'DATA_FLOW', 'SIMPLE');
            }
            return webflowValue.charAt(0).toUpperCase() + webflowValue.slice(1).toLowerCase();
        ...
        convertTipoVeiculo(webflowValue) {
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'DATA_TRACE', 'Convertendo tipo de veículo', { 
                    original: webflowValue, 
                    tipo 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            for (const tipoCarro of tiposCarro) {
                if (tipo.includes(tipoCarro) || tipoCarro.includes(tipo)) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', 'Tipo de veículo reconhecido como carro', { 
                            tipo, 
                            resultado: 'carro' 
                        }, 'DATA_FLOW', 'MEDIUM');
                    }
            ...
            for (const tipoMoto of tiposMoto) {
                if (tipo.includes(tipoMoto) || tipoMoto.includes(tipo)) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'DATA_TRACE', 'Tipo de veículo reconhecido como moto', { 
                            tipo, 
                            resultado: 'moto' 
                        }, 'DATA_FLOW', 'MEDIUM');
                    }
            ...
            if (window.novo_log) {
                window.novo_log('WARN', 'DATA_TRACE', 'Tipo de veículo não reconhecido, usando carro como padrão', { 
                    webflowValue 
                }, 'DATA_FLOW', 'SIMPLE');
            }
            return 'carro';
        ...
        async handleFormSubmit(form) {
            try {
                if (window.novo_log) {
                    window.novo_log('INFO', 'RPA', 'Iniciando processo RPA', null, 'OPERATION', 'SIMPLE');
                }
                ...
                // Chamar API RPA
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Enviando dados para API', { 
                        campos: Object.keys(formData).length 
                    }, 'DATA_FLOW', 'MEDIUM');
                    window.novo_log('TRACE', 'RPA', 'JSON sendo enviado para API', formData, 'DATA_FLOW', 'VERBOSE');
                }
                
                const response = await fetch(`${RPA_API_BASE_URL}/api/rpa/start`, { // FASE 2 - Correção ALTA: usar constante configurável
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Resposta da API recebida', { 
                ...
                if (result.success && result.session_id) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('INFO', 'RPA', 'Session ID recebido', { sessionId: this.sessionId }, 'OPERATION', 'MEDIUM');
                    }
                ...
                if (window.novo_log) {
                    window.novo_log('ERROR', 'RPA', 'Erro no processo', error, 'ERROR_HANDLING', 'MEDIUM');
                }
                this.updateButtonLoading(false);
        ...
        initializeProgressModal() {
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Inicializando modal de progresso', null, 'OPERATION', 'SIMPLE');
            }
            ...
            setTimeout(() => {
                if (window.ProgressModalRPA) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'RPA', 'Modal de progresso inicializado', null, 'OPERATION', 'SIMPLE');
                    }
                } else {
                    if (window.novo_log) {
                        window.novo_log('ERROR', 'RPA', 'ProgressModalRPA não encontrado', { 
                            classesDisponiveis: Object.keys(window).filter(key => key.includes('Modal')) 
                        }, 'ERROR_HANDLING', 'MEDIUM');
                    }
                }
            }, 200);
        ...
        async validateFormData(formData) {
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'validateFormData iniciado', { 
                    campos: Object.keys(formData).length 
                }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Iniciando validações paralelas', null, 'DATA_FLOW', 'SIMPLE');
            }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Telefone extraído', { ddd, celular }, 'DATA_FLOW', 'MEDIUM');
            }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Resultados das validações', {
                    cpf: cpfResult.ok,
                    cep: cepResult.ok,
                    placa: placaResult.ok,
                    celular: celularResult.ok,
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'VALIDACAO', 'Resultado final isValid', { isValid }, 'DATA_FLOW', 'SIMPLE');
            }
            
            // Auto-preenchimento se válido
            if (placaResult.ok && placaResult.parsed) {
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'VALIDACAO', 'Auto-preenchendo dados da placa', placaResult.parsed, 'DATA_FLOW', 'MEDIUM');
                }
                // Auto-preenchimento de campos da placa
            }
            
            if (cpfResult.ok && cpfResult.parsed && validator.config.VALIDAR_PH3A) {
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'VALIDACAO', 'Auto-preenchendo dados do CPF', cpfResult.parsed, 'DATA_FLOW', 'MEDIUM');
                }
            ...
            if (window.novo_log) {
                window.novo_log('TRACE', 'VALIDACAO', 'Retornando resultado final', result, 'DATA_FLOW', 'MEDIUM');
            }
            return result;
        }
        ...
        async showValidationAlert(errors) {
            if (window.novo_log) {
                window.novo_log('DEBUG', 'VALIDACAO', 'showValidationAlert chamado', { 
                    erros: Object.keys(errors).filter(k => !errors[k].ok) 
                }, 'ERROR_HANDLING', 'MEDIUM');
            }
            ...
            if (!errors.cpf.ok) {
                ...
                if (window.novo_log) {
                    window.novo_log('WARN', 'VALIDACAO', 'CPF inválido', errors.cpf, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
            if (!errors.cep.ok) {
                errorLines += "• CEP inválido\n";
                if (window.novo_log) {
                    window.novo_log('WARN', 'VALIDACAO', 'CEP inválido', errors.cep, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
            if (!errors.placa.ok) {
                errorLines += "• Placa inválida\n";
                if (window.novo_log) {
                    window.novo_log('WARN', 'VALIDACAO', 'Placa inválida', errors.placa, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
            if (!errors.celular.ok) {
                errorLines += "• Celular inválido\n";
                if (window.novo_log) {
                    window.novo_log('WARN', 'VALIDACAO', 'Celular inválido', errors.celular, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
            if (!errors.email.ok) {
                errorLines += "• E-mail inválido\n";
                if (window.novo_log) {
                    window.novo_log('WARN', 'VALIDACAO', 'Email inválido', errors.email, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
            
            if (window.novo_log) {
                window.novo_log('INFO', 'VALIDACAO', 'Mostrando SweetAlert de validação', { 
                    errorLines 
                }, 'UI', 'SIMPLE');
            }
            ...
            if (result.isConfirmed) {
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'VALIDACAO', 'Usuário escolheu corrigir, focando campo com erro', null, 'OPERATION', 'SIMPLE');
                }
                ...
                if (window.novo_log) {
                    window.novo_log('INFO', 'VALIDACAO', 'Usuário escolheu prosseguir, redirecionando', null, 'OPERATION', 'SIMPLE');
                }
                // NÃO executar RPA - redirecionar para página de sucesso
                window.location.href = SUCCESS_PAGE_URL; // FASE 2 - Correção ALTA: usar constante configurável
            }
        }
        ...
        setFieldValue(id, val) {
            ...
            if (field) {
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', `Campo ${id} preenchido`, { valor: val }, 'DATA_FLOW', 'MEDIUM');
                }
            } else {
                if (window.novo_log) {
                    window.novo_log('WARN', 'RPA', `Campo ${id} não encontrado`, null, 'DATA_FLOW', 'SIMPLE');
                }
            }
        }
        ...
        async executeWebflowWebhooks(form, formData) {
            window.novo_log('INFO', 'RPA', '🔗 Executando webhooks do Webflow...');
            ...
            try {
                ...
                window.novo_log('INFO', 'RPA', '✅ Todos os webhooks executados com sucesso');
                
            } catch (error) {
                window.novo_log('WARN', 'RPA', '⚠️ Erro ao executar webhooks', { error: error });
        ...
        async sendToWebflow(formData) {
            try {
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Webflow webhook executado', null, 'DATA_FLOW', 'SIMPLE');
                }
            } catch (error) {
                if (window.novo_log) {
                    window.novo_log('WARN', 'RPA', 'Erro no webhook Webflow', { error: error.message }, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
        }
        ...
        async sendToWebhookSite(formData) {
            if (!WEBHOOK_SITE_URL) {
                if (window.novo_log) {
                    window.novo_log('WARN', 'RPA', 'WEBHOOK_SITE_URL não configurado, pulando webhook.site', null, 'ERROR_HANDLING', 'SIMPLE');
                }
            ...
            try {
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'RPA', 'Webhook.site executado', null, 'DATA_FLOW', 'SIMPLE');
                }
            } catch (error) {
                if (window.novo_log) {
                    window.novo_log('WARN', 'RPA', 'Erro no webhook.site', { error: error.message }, 'ERROR_HANDLING', 'SIMPLE');
                }
            }
        }
        ...
        */
        ...
        ensureFontAwesomeLoaded() {
            return new Promise((resolve) => {
                ...
                if (existingFA) {
                    if (window.novo_log) {
                        window.novo_log('TRACE', 'UI_TRACE', 'Font Awesome já carregado', null, 'UI', 'SIMPLE');
                    }
                    // Aguardar um pouco para garantir que os estilos foram aplicados
                    setTimeout(() => {
                        if (window.novo_log) {
                            window.novo_log('TRACE', 'UI_TRACE', 'Font Awesome verificado e pronto', null, 'UI', 'SIMPLE');
                        }
                        resolve();
                    }, 100);
                    return;
                ...
                if (window.novo_log) {
                    window.novo_log('DEBUG', 'UI_TRACE', 'Carregando Font Awesome', null, 'UI', 'SIMPLE');
                }
                ...
                fontAwesome.onload = () => {
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'UI_TRACE', 'Font Awesome carregado com sucesso', null, 'UI', 'SIMPLE');
                    }
                    // Aguardar um pouco para garantir que os estilos foram aplicados
                    setTimeout(() => {
                        if (window.novo_log) {
                            window.novo_log('DEBUG', 'UI_TRACE', 'Font Awesome aplicado e pronto', null, 'UI', 'SIMPLE');
                        }
                        resolve();
                    }, 200);
                };
                ...
                fontAwesome.onerror = () => {
                    if (window.novo_log) {
                        window.novo_log('WARN', 'UI', 'Erro ao carregar Font Awesome', error, 'ERROR_HANDLING', 'SIMPLE');
                    }
                    resolve();
                };
        ...
        openProgressModal() {
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Criando modal de progresso', null, 'UI', 'SIMPLE');
            }
            ...
            setTimeout(() => {
                ...
                if (spinnerContainer) {
                    ...
                    if (window.novo_log) {
                        window.novo_log('DEBUG', 'UI_TRACE', 'Spinner timer container exibido', null, 'UI', 'SIMPLE');
                    }
            ...
            if (window.novo_log) {
                window.novo_log('DEBUG', 'RPA', 'Modal HTML criado - aguardando Session ID', null, 'OPERATION', 'SIMPLE');
            }
        }
    }
    ...
    if (!document.querySelector('link[href*="font-awesome"]') && !document.querySelector('link[href*="fontawesome"]')) {
        ...
        document.head.appendChild(fontAwesome);
        if (window.novo_log) {
            window.novo_log('DEBUG', 'INIT', 'Font Awesome carregado', null, 'INIT', 'SIMPLE');
        }
    }
    
    // Aguardar SweetAlert2 carregar
    if (!document.querySelector('script[src*="sweetalert2"]')) {
        ...
        if (window.novo_log) {
            window.novo_log('DEBUG', 'INIT', 'SweetAlert2 v11.14.0 carregado', null, 'INIT', 'SIMPLE');
        }
    }
    ...
    if (window.novo_log) {
        window.novo_log('INFO', 'INIT', 'Webflow Injection Complete V6.13.0 carregado com sucesso', {
            features: [
                'SpinnerTimer integrado com ciclo de vida do RPA',
                'Validação completa de formulário implementada',
                'SweetAlert com opção "Prosseguir assim mesmo"',
                'Auto-preenchimento de campos (MARCA/ANO/TIPO)',
                'Redirecionamento para página de sucesso em caso de dados inválidos'
            ]
        }, 'INIT', 'MEDIUM');
    }
