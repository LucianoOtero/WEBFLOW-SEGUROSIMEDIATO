 ...
 * 
 * Webhook de produção para integração Webflow + OctaDesk
...
// ==================== CONFIGURAÇÃO CORS ====================
// Usar getCorsOrigins() de config.php
...
// Função HTTP helper para OctaDesk (PRODUÇÃO)
function octa_request($method, $url, $body = null) {
    ...
    logProdWebhook('OCTA_REQ', ['method'=>$method, 'url'=>$url, 'body'=>$body]);
    $ch = curl_init($url);
    $opts = [
    ...
    $err = curl_error($ch);
    curl_close($ch);
    logProdWebhook('OCTA_RES', ['http'=>$http, 'url'=>$url, 'err'=>$err, 'body_head'=>substr((string)$resp, 0, 600)]);
...
// Função para enviar template WhatsApp via OctaDesk (PRODUÇÃO)
...
function sendToOctaDesk($data)
{
    ...
    if (!$foneE164) {
        logProdWebhook('phone_validation_error', ['phone' => $phone, 'ddd' => $ddd, 'celular' => $celular], false);
        return [
            'http_code' => 422,
            'response' => '',
    ...
    logProdWebhook('octadesk_send_template_payload', [
        'payload' => $payloadSend,
        'phone_e164' => $foneE164,
...
function processWebflowWebhook()
{
    ...
    logProdWebhook('contact_data_mapped', $contactData, true);

    // Validar telefone obrigatório (ddd e celular)
    if (empty($ddd) || empty($celular)) {
        logProdWebhook('validation_error', [
            'field' => 'telefone', 
    ...
    // Enviar template WhatsApp via OctaDesk (PRODUÇÃO)
    $octaResult = sendToOctaDesk($contactData);

    if ($octaResult['success']) {
        logProdWebhook('webhook_success', [
            'form_name' => $formName,
...
try {
    ...
    logProdWebhook('webhook_exception', [
        'error' => $e->getMessage(),
        'file' => $e->getFile(),
        'line' => $e->getLine(),
